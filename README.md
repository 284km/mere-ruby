...
```sh
./run_corpus.sh                                  # 198 programs, byte-for-byte
...
and under `./mere-ruby` and diffs the output byte-for-byte: **198 programs,
198 identical**. The corpus covers the semantic corners above. A deliberate negative control (lossy float
...
The record covers **1221 spec files** across 31 groups: **993 MATCH, 224 DIFF,
...
(23 files): a name that is not there, which is missing surface rather than
